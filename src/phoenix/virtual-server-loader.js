...
function _isServiceWorkerLoaderPage() {
    ...
    const indexUrl = `${location.origin}/index.html`,
        ...
        playwrightDevURL = `${location.origin}/src/`,
        playwrightDistTestURL = `${location.origin}/dist-test/src/`,
        currentURL = _getBaseURL(),
        isLoopbackTestServer = location.port === "5000" &&
    ...
    return (currentURL === baseUrl || currentURL === indexUrl || currentURL === devURL || currentURL === distTestURL ||
        (isLoopbackTestServer && currentURL === playwrightDevURL &&
            window.Phoenix.browser.desktop.isChromeBased) ||
        (isLoopbackTestServer && currentURL === playwrightDistTestURL &&
            window.Phoenix.browser.desktop.isChromeBased));
