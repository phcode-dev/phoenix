...
define({
  ...
  "INCLUDE_FILE_FILTER": "{0} を含める",
  "EXCLUDE_FILE_FILTER_DROPDOWN": "除外する",
  "INCLUDE_FILE_FILTER_DROPDOWN": "含む",
  "EDIT_FILE_FILTER": "編集…",
  "FILE_FILTER_DIALOG": "除外フィルターの編集",
  "FILE_FILTER_INSTRUCTIONS": "次のいずれかに一致するファイルとフォルダーを除外します。<a href='{0}' title=\"{0}\">ワイルドカードは無視されます</a>。各文字列を新しい行に入力します。例えば<ul><li><code>*.js</code> <code>a/b/x.js</code> 、 <code>xyx.js</code>などに一致します。</li><li> <code>./*.css</code>プロジェクト ルートの<code>x.css</code>のみと一致しますが、サブ フォルダーの<code>y/x.css</code>とは一致しません。</li></ul>",
  "FILE_FILTER_INSTRUCTIONS_INCLUDE": "次のいずれかに一致するファイルとフォルダーのみを含めます。<a href='{0}' title=\"{0}\">ワイルドカードは無視されます</a>。各文字列を新しい行に入力します。例えば<ul><li><code>*.js</code> <code>a/b/x.js</code> 、 <code>xyx.js</code>などに一致します。</li><li> <code>./*.css</code>プロジェクト ルートの<code>x.css</code>のみと一致しますが、サブ フォルダーの<code>y/x.css</code>とは一致しません。</li></ul>",
  ...
  "CMD_KEYBOARD_NAV_OVERLAY": "UIナビゲーションモード",
  "NAVIGATE_MENU": "ナビゲート",
  ...
  "HEALTH_DATA_NOTIFICATION_MESSAGE": "{APP_NAME} を改善するために、{APP_NAME} の使用方法に関する限定的な<strong>仮名の</strong>統計情報とエラー レポートを phcode.dev に定期的に送信しています。この情報は、プライバシーを保護しながら、機能の優先順位付け、バグの発見、ユーザビリティの問題の発見に役立ちます。<br><br> <strong>[ヘルプ] &gt; [正常性レポート]</strong>を選択して、データを表示するか、データを共有しないように選択できます。<br><br>",
  "HEALTH_DATA_PREVIEW": "{APP_NAME} の正常性レポート",
  "HEALTH_DATA_PREVIEW_INTRO": "<p>{APP_NAME} を改善するために、{APP_NAME} の使用方法に関する限定的な<strong>仮名の</strong>統計情報とエラー レポートを phcode.dev に定期的に送信しています。この情報は、機能の優先順位付け、バグの発見、およびユーザビリティの問題の特定に役立ちます。</p><p>以下は、有効になっている<em>場合</em>に次回のヘルス レポートで送信されるデータのプレビューです。 (&#39;Caught Critical error&#39; とマークされたエラー ログについては、開発者コンソールも参照してください。)</p>",
  ...
  "ATTENTION_SAFARI_USERS_MESSAGE": "<span>Safari は、サイトが 7 日以内に再度アクセスされない場合、Web サイトのデータを自動的に削除します。 {APP_NAME} はブラウザ ストレージに依存してプロジェクトを保存していますが、このポリシーによりプロジェクトが削除される可能性があります。<br/><br/><strong>データの損失を防ぐため</strong>、タブを閉じる前にプロジェクトを<strong>ダウンロードする</strong>か、<strong>今後の Web およびネイティブ Mac/iOS アプリ</strong>にご注目ください。</span>",
