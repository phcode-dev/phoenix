...
const MINOR_VERSION_BUMP = 2, MAJOR_VERSION_BUMP = 0;
function _patchBumpConfigFile(fileName, whichVersion) {
    let config = JSON.parse(fs.readFileSync(fileName, 'utf8'));
    let version = config.apiVersion .split("."); // ["3","0","0"]
    version[whichVersion] = "" + (parseInt(version[whichVersion]) + 1); // ["3","0","1"]
...
function patchVersionBump() {
    return new Promise((resolve)=> {
        _patchBumpConfigFile('./package.json', MINOR_VERSION_BUMP);
        _patchBumpConfigFile('./src/config.json', MINOR_VERSION_BUMP);
        resolve();
    });
}
...
function majorVersionBump() {
    return new Promise((resolve)=> {
        _patchBumpConfigFile('./package.json', MAJOR_VERSION_BUMP);
        _patchBumpConfigFile('./src/config.json', MAJOR_VERSION_BUMP);
        resolve();
    });
}
